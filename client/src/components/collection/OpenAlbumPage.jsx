...
function OpenAlbumPage({ usersCardCollectionArray }) {
  ...
  return (
    <div className='bg-green-400 grid grid-cols-a1a'>
      <div onClick={prevPageCards} className='grid items-center px-2'>£</div>
      <section className='grid grid-rows-a11a bg-black px-2 gap-4'>
        <div className='flex justify-between px-1 items-center h-fit outline outline-1 outline-white text-blue-500'>
          <div>{currentIndex - 10 + 1} / {currentIndex}</div>
        ...
        <div className='h-fit outline outline-1 outline-white'>Total cards: 500</div>
      </section>
      <div onClick={nextPageCards} className='grid items-center px-2'>£</div>
    </div>
  );
}
